    ...
    public class HurtBox : IBox
    {
        ...
        public void GetHit(ActorModel attacker, Transform atkTransform, AttackBase atk)
        {
            ...
            if (model.currentBehavior.atkBlockingAngle <= angle)
            {
                ...
                ApplyHitInfo(attacker, atkTransform, atk);
            }
            else
            {             
                // 反击成功
        ...
        void ExecuteHurtBehavior(AttackBase currentAtk, Transform atkTransform)
        {
            ...
            //{
                ...
                {
                    case AttackImpactType.Normal: //普通攻击
                        getHitBehavior = model.GetBehavior("Hurt_Normal"); // todo change get hit to a standalone behavior
                        break;
                    case AttackImpactType.KnockDown: // 击倒在地
                        getHitBehavior = model.GetBehavior("Hurt_KnockDown"); // todo change get hit to a standalone behavior
                        break;
                    case AttackImpactType.HitFly: // 击飞
                        getHitBehavior = model.GetBehavior("Hurt_Fly"); // todo change get hit to a standalone behavior
                        break;
                }
            //}
            //else
            //{
            //    // 在空中受到攻击
            //    getHitBehavior = model.GetBehavior("HitOnFly"); // todo change get hit to a standalone behavior
            //}
