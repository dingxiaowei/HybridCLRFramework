...
namespace libx
{
    public sealed class Assets : MonoBehaviour
    {
        ...
        public static AssetRequest LoadAssetAsync(string path, Type type, Action<AssetRequest> callBack)
        {
            var assetRequest = LoadAsset(path, type, true);
            assetRequest.completed = callBack;
            return assetRequest;
        }

        public static AssetRequest LoadPrefabAsync(string path, Action<AssetRequest> callBack)
        {
            var assetRequest = LoadAsset(path, typeof(GameObject), true);
            assetRequest.completed = callBack;
            return assetRequest;
        }
